...
describe "the symphony of things" do
  ...
  before(:all) do
    class Ctrlr < WhalesController::Base
      def route_render
  ...
  describe "routes and params" do
    it "route instantiates controller and calls invoke action" do
      route = WhalesDispatch::Route.new(Regexp.new("^/statuses/(?<id>\\d+)$"), :get, Ctrlr, :route_render)
    ...
    it "route adds to params" do
      route = WhalesDispatch::Route.new(Regexp.new("^/statuses/(?<id>\\d+)$"), :get, Ctrlr, :route_does_params)
      allow(req).to receive(:path) { "/statuses/1" }
      allow(req).to receive(:request_method) { :get }
      route.run(req, res)
  ...
  describe "controller sessions" do
    ...
    it "exposes a session via the session method" do
      expect(ctrlr.session).to be_instance_of(WhalesDispatch::Session)
    end
