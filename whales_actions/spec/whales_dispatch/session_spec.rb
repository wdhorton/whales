...
describe WhalesDispatch::Session do
  ...
  let(:cook) { WEBrick::Cookie.new('_whales_app', { xyz: 'abc' }.to_json) }
  ...
  describe "#store_session" do
    context "without cookies in request" do
      ...
      it "adds new cookie with '_whales_app' name to response" do
        cookie = res.cookies.find { |c| c.name == '_whales_app' }
    ...
    context "with cookies in request" do
      before(:each) do
        cook = WEBrick::Cookie.new('_whales_app', { pho: "soup" }.to_json)
        req.cookies << cook
      end
