...
describe WhalesDispatch::Params do
  before(:all) do
  ...
  it "handles an empty request" do
    expect { WhalesDispatch::Params.new(req) }.to_not raise_error
  end

  context "query string" do
    it "handles single key and value" do
      req.query_string = "key=val"
      params = WhalesDispatch::Params.new(req)
  ...
  context "route params" do
    it "handles route params" do
      params = WhalesDispatch::Params.new(req, {"id" => 5, "user_id" => 22})
  ...
  context "indifferent access" do
    it "responds to string and symbol keys when stored as a string" do
      params = WhalesDispatch::Params.new(req, {"id" => 5})
      expect(params["id"]).to eq(5)
      expect(params[:id]).to eq(5)
    end
  ...
  #       expect { params.require("key2") }.to raise_error(WhalesDispatch::Params::AttributeNotFoundError)
  #     end
  #   end
  #
...
describe 'WhalesController::Base#initialize' do
  before(:all) do
    class CatsController < WhalesController::Base
      def index
        @cats = ["Gizmo"]
  ...
  context '#initialize' do
    it "sets params var to new Params object" do
      expect(cats_controller.params).to be_instance_of(WhalesDispatch::Params)
    end
  end
end
